function objective_edge_expression(::RoutingModel, obj_edge::Val, ::Edge, ::Dict{Edge, Float64})
    error("Edge-wise objective function not implemented: $obj_edge")
end

function objective_edge_expression(rm::RoutingModel, obj_edge::Val, e::Edge)
...
function objective_edge_expression(rm::RoutingModel, ::Val{Load}, e::Edge, dm::Dict{Edge, Float64})
    return total_flow_in_edge(rm, e, dm) / get_prop(rm.data.g, e, :capacity)
end

function objective_edge_expression(rm::RoutingModel, ::Val{KleinrockLoad}, e::Edge, dm::Dict{Edge, Float64})
    ...
    load_e = objective_edge_expression(rm, Val(Load), e, dm)
...
function objective_edge_expression(rm::RoutingModel, ::Val{FortzThorupLoad}, e::Edge, dm::Dict{Edge, Float64})

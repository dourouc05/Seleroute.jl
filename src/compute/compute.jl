"""
Based on an opaque `RoutingData` object `rd` and a solver (for instance,
...
function compute_routing(rd::RoutingData)
    ...
    # TODO: everywhere if iterative, add parameter to remember all generated
    # routings or not (store it in RoutingData?).
    mt = rd.model_type
...
function compute_routing(::RoutingData, edge_obj::EdgeWiseObjectiveFunction,
                         ...
                         unc::UncertaintyHandling, uncparams::UncertainParameters)
    # When dispatch fails, use this fallback.
    msg = "Model type not yet implemented: ModelType($edge_obj, $agg_obj, $type, "
    msg *= "$cg, $algo, $unc, $uncparams).\n"
